...
# /* FIX: Removed ::before pseudo-element from stAppViewContainer — it creates
#    a stacking context that fights Streamlit's own z-index layers and causes
#    widgets/tooltips to render behind the overlay on some browsers. The
#    decorative gradients are now on .dash-bg which sits below all content. */
# .dash-bg {
#     position: fixed;
...
# /* ── Metric cards ──
#    FIX: Changed from 5-column fixed grid to auto-fit with a sensible min-width.
#    The old grid collapsed from 5 → 2 cols at 1200px, leaving a huge gap at
#    mid-widths (900–1200px). auto-fit fills the row naturally at every width.
#    min 180px keeps cards readable; 1fr lets them grow to fill space. ── */
# .metric-grid {
#     display: grid;
...
# }
# /* FIX: Removed hardcoded font-size from .metric-card-value — the old code
#    applied font-size:1.55rem inline on the "Most Idle" card only, which made
#    it visually inconsistent. All sizing is now controlled here with a single
#    clamp() so values shrink gracefully when the machine ID is long. */
# .metric-card-value {
#     font-family: 'Rajdhani', sans-serif;
...
# # FIX: Decorative background div rendered as first element instead of
# # a ::before on the app container — avoids stacking context conflicts.
...
# # CONNECT DB
# # =========================
# # FIX: TTL raised from 2s → 10s. A 2s TTL caused near-constant cache misses
# # and DB reconnects on every user interaction. 10s is still "live" for an
# # equipment dashboard while being far more stable.
...
#     # FIX: Was using hardcoded hex values that clashed with the dark theme CSS
#     # variables. Replaced with colours that match the --danger / --bg-card
#     # palette defined in :root so INACTIVE rows are clearly visible.
...
# # LOAD DATA
# # =========================
# # FIX: load_data now returns (df, error) tuple so DB failures are surfaced
# # without the app silently showing a "Live Pipeline Running" success badge.
# df, db_error = load_data()

# # =========================
# # LIVE STATUS — shown conditionally based on actual DB result
# # FIX: Previously st.success("Live Pipeline Running") was always displayed
# # regardless of whether the DB connection succeeded or failed.
# # =========================
# if db_error:
#     st.error(f"⛔ Database connection failed: {db_error}")
# elif df.empty:
#     st.warning("⚠️ Connected — no records in equipment_tracking yet.")
#     st.info("Make sure the tracker has written records to PostgreSQL.")
# else:
#     st.success("🟢 Live Pipeline Running")
...
#     # FIX: Removed hardcoded font-size:1.55rem inline style on "Most Idle"
#     # value — all cards now use the same .metric-card-value class with clamp().
#     # Unit suffix moved into a .unit span styled via CSS (no inline font-size).
#     metrics_html = f"""
...
#             <div class="metric-card-value warning">{most_idle_id}</div>
#         </div>
#     </div>
#     """
...
#      # Optional: cleaner numbers
#     display_df["Idle Session (s)"] = display_df["Idle Session (s)"].round(2)
#     display_df["Total Idle (s)"] = display_df["Total Idle (s)"].round(2)
#     display_df["Total Active (s)"] = display_df["Total Active (s)"].round(2)
...
#     st.dataframe(
#     display_df.style.apply(highlight_idle_rows, axis=1),
...
#     st.success("🎥 Video stream active")

#     # FIX: The video shell open/close tags were split across st.markdown calls
#     # with st.video() in between. If st.video() raised an exception the closing
#     # </div></div> was never emitted, breaking the DOM for everything below.
#     # Now the shell header is a standalone div and st.video runs in a clean
#     # try/except — no dangling tags regardless of outcome.
#     st.markdown(f"""
...
# # =========================
# # AUTO REFRESH
# # FIX: Moved time.sleep() to the very bottom, after all rendering is complete.
# # The original placement was before data load — sleeping mid-render blocked
# # the entire Streamlit thread and made the UI appear frozen for 8 seconds.
# # Now the page renders fully first, then sleeps and reruns cleanly.
# # =========================
# if auto_refresh:
#     time.sleep(8)
#     st.rerun()
