...
def machine_cards(df):
    cards_html = '<div class="machine-grid">'
    for _, row in df.iterrows():
        is_active = str(row.get("current_state", "")).upper() == "ACTIVE"
        mc_class  = "mc-active" if is_active else "mc-idle"
        st_class  = "st-active" if is_active else "st-idle"
        st_label  = "ACTIVE" if is_active else "IDLE"
        ...
        util = float(row.get("utilization_percent", 0))
        util_color = "#39ff14" if util >= 60 else "#ffaa00" if util >= 30 else "#ff3b5c"
        util_glow  = util_color

        idle_sec   = float(row.get("current_idle_session_seconds", 0))
        total_idle = float(row.get("total_idle_seconds", 0))
        total_act  = float(row.get("total_active_seconds", 0))
        idle_color = "red" if idle_sec > 30 else "orange" if idle_sec > 5 else "green"

        cards_html += f"""
        <div class="machine-card {mc_class}">
            <div class="mc-header">
                <div class="mc-id">{row.get('equipment_id','—')}</div>
                <div class="mc-status {st_class}">{st_label}</div>
            </div>
            <div class="mc-rows">
                <div class="mc-row">
                ...
                <div class="mc-row">
                    <span class="mc-key">Activity</span>
                    <span class="mc-val">{row.get('current_activity','—')}</span>
                </div>
                <div class="mc-row">
                    <span class="mc-key">Motion source</span>
                    <span class="mc-val cyan">{row.get('motion_source','—')}</span>
                </div>
                <div class="mc-row">
                    <span class="mc-key">Idle session</span>
                    <span class="mc-val {idle_color}">{idle_sec:.1f}s</span>
                ...
                <div class="mc-row">
                    <span class="mc-key">Total active</span>
                    <span class="mc-val green">{total_act:.1f}s</span>
                </div>
                <div class="mc-row">
                    <span class="mc-key">Total idle</span>
                    <span class="mc-val orange">{total_idle:.1f}s</span>
                </div>
                <div class="mc-row">
                    <span class="mc-key">Utilization</span>
                    <span class="mc-val" style="color:{util_color};text-shadow:0 0 6px {util_glow}88">{util:.1f}%</span>
...
if db_error:
    ...
    df_grouped = df.groupby("equipment_id", as_index=False).agg({
        "track_id": "last",
        "equipment_class": "first",
        "current_state": "first",
        "current_activity": "first",
        "motion_source": "first",
        "current_idle_session_seconds": "max",
        "total_tracked_seconds": "max",
        "total_idle_seconds": "max",
        "total_active_seconds": "max",
        "utilization_percent": "max"
    })
    df_grouped = df_grouped.sort_values("equipment_id").reset_index(drop=True)

    total_machines = df_grouped["equipment_id"].nunique()
    avg_util  = df_grouped["utilization_percent"].mean() if total_machines > 0 else 0
    most_idle = (
        df_grouped.sort_values("total_idle_seconds", ascending=False).iloc[0]["equipment_id"]
        if not df_grouped.empty else "N/A"
    )
    active_count = int((df_grouped["current_state"] == "ACTIVE").sum())
    idle_count = int((df_grouped["current_state"] == "INACTIVE").sum())
    ...
    NUMERIC_COLS = [
        "Idle Session (s)", "Total Tracked (s)", "Total Idle (s)",
        "Total Active (s)", "Utilization (%)"
    ]

    display_df = (
        df_grouped[df_grouped["equipment_id"].notna() & (df_grouped["equipment_id"].astype(str).str.strip() != "")]
        [[
            "equipment_id", "track_id", "equipment_class", "current_state", "current_activity",
            "motion_source", "current_idle_session_seconds", "total_tracked_seconds",
            "total_idle_seconds", "total_active_seconds", "utilization_percent"
        ]]
        .rename(columns={
            ...
            "current_idle_session_seconds":  "Idle Session (s)",
            "total_tracked_seconds":         "Total Tracked (s)",
            "total_idle_seconds":            "Total Idle (s)",
            "total_active_seconds":          "Total Active (s)",
            "utilization_percent":           "Utilization (%)",
        })
        .reset_index(drop=True)
    )
    ...
    st.markdown('<div class="section-label" style="margin-top:1.5rem">Alert Feed</div>', unsafe_allow_html=True)
    idle_alerts = df_grouped[df_grouped["current_idle_session_seconds"] > 5]
    if not idle_alerts.empty:
        alerts_html = ""
        for _, row in idle_alerts.iterrows():
            alerts_html += f"""
            <div class="alert-row">
                ⚠ &nbsp;Machine <span class="ar-machine">{row['equipment_id']}</span>
                has been idle for <span class="ar-time">{row['current_idle_session_seconds']:.1f}s</span>
                &nbsp;·&nbsp; <span class="ar-act">{row['current_activity']}</span>
            </div>"""
        st.markdown(alerts_html, unsafe_allow_html=True)
    else:
        neon_banner("success", "✅ All machines operating within normal parameters", "OK")
    ...
    """, unsafe_allow_html=True)
    st.bar_chart(
        df_grouped[["equipment_id", "utilization_percent"]].set_index("equipment_id"),
        height=260
    )
    ...
    <div class="video-neon-shell">
        <div class="video-neon-inner">
            <div class="video-neon-header">
                <div class="video-rec"></div>
                <span class="video-neon-title">CV Tracking Feed</span>
                <span class="video-neon-meta">MP4 &nbsp;·&nbsp; {size_mb:.1f} MB &nbsp;·&nbsp; LIVE</span>
            </div>
            <div class="video-neon-body">
    """, unsafe_allow_html=True)
